...
*regress `vio' south
	...
	[aw=pop_prov]
	if sample_all==1, robust cluster(province);
*test south popdensity6061 south_popdensity6061 $x_weather $x_oth $x_elev;
...
*areg `var' `vio'
	[aw=pop_tot]
	if (popdensity6061>=145 & popdensity6061~=.), a(province) robust cluster(province);
bys south: regress `var' `vio'
	[aw=pop_tot]
	if popdensity6061<145, robust cluster(province);
bys south: areg `var' `vio'
	[aw=pop_tot]
	if popdensity6061<145, a(province) robust cluster(province);
