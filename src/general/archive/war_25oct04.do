...
global x_weather = "pre_avg pre_sd tmp_avg tmp_sd";
...
replace sample_all = 0 if 
	(poverty_p0==. | tot_bmr==.
	| popdensity6061==. | south==. | pre_avg==.
	| tot_bmr==0 | tot_bmr_per==.);

gen sample_trim = sample_all;
replace sample_trim = 0 if (tot_bmr>170000 | tot_bmr<7);

gen sample_restrict = sample_all;
replace sample_restrict = 0 if rlpcex1==.;
...
desc
	...
	rlpcex1_9398 rlpcex1 rlpcex1_93 CONSGROWTHPC
	avg_ill4wks literate numerate
	educyr98_head educyr98father educyr98mother
	farm notbornhere yrshere 
	$ord0 $ord1 $ord2 $ord3 /* $ord4 $ord5 $ord6 $ord7 $ord8 */
	tot_bmr* tot_bomb* war_f1 war_f2
	$x_weather $x_oth $x_elev /* $x_soil1 $x_soil2 $x_slope */
	popdensity* paddyyield* births* south*
	oxen* buffalos*
	pop_tot pop_prov sample*;
...
bys south: summ
	...
	popdensity* paddyyield* births* south*
	oxen* buffalos*
	pop_tot pop_prov  sample* [aw=pop_tot]
	if sample_all==1;
...
foreach vio in
	tot_bmr tot_bmr_per tot_bmr_hi /* General_Purpose war_f1 */
{;
use temp2;
regress `vio' south
	[aw=pop_prov], robust;
...
regress `vio' 
	...
	oxen1960_n buffalos1960_n
	if south==0
	[aw=pop_prov], robust;
test popdensity6061 paddyyield6061 oxen1960_n buffalos1960_n;
regress `vio' 
	popdensity6061 paddyyield6061
	educyr98father
	[aw=pop_prov]
	if south==0, robust;
test popdensity6061 paddyyield6061 educyr98father;
regress `vio' 
	popdensity6061 paddyyield6061
	educyr98father
	[aw=pop_prov]
...
*regress `vio' south
	popdensity6061 south_popdensity6061
	[aw=pop_prov], robust;

twoway (scatter `vio' popdensity6061, ms(Oh)) (lfit `vio' popdensity6061 [aw=pop_prov]), saving(`vio'_popdensity_lin, replace);
twoway (scatter `vio' popdensity6061, ms(Oh)) (lfit `vio' popdensity6061 [aw=pop_prov]) if south==0, saving(`vio'_popdensity_linN, replace);
twoway (scatter `vio' popdensity6061, ms(Oh)) (lfit `vio' popdensity6061 [aw=pop_prov]) if south==1, saving(`vio'_popdensity_linS, replace);
*twoway (scatter popdensity6061 `vio', ms(Oh)) (lfit popdensity6061 `vio' [aw=pop_prov]), saving(popdensity_`vio'_lin, replace);
clear;
...
regress `vio'
	popdensity6061
	$x_weather $x_oth $x_elev
	[aw=pop_prov]
	if sample_all==1 & south==0, robust cluster(province);
test popdensity6061 $x_weather $x_oth $x_elev;
regress `vio'
	popdensity6061
	$x_weather $x_oth $x_elev
	[aw=pop_prov]
	if sample_all==1 & south==1, robust cluster(province);
test popdensity6061 $x_weather $x_oth $x_elev;
...
regress `vio'
	...
	[aw=pop_prov]
	if sample_all==1 & south==0, robust cluster(province);
test popdensity6061 educyr98father $x_weather $x_oth $x_elev;
regress `vio'
	popdensity6061
	educyr98father
	$x_weather $x_oth $x_elev
...
areg `vio' 
	$x_weather $x_oth $x_elev
	[aw=pop_prov]
	if sample_all==1 & south==0, a(province) robust cluster(province);
test $x_weather $x_oth $x_elev;
areg `vio' 
	$x_weather $x_oth $x_elev
	[aw=pop_prov]
	if sample_all==1 & south==1, a(province) robust cluster(province);
test $x_weather $x_oth $x_elev;
...
/* ESTIMATED BOMBING IMPACTS */;
foreach vio in
	tot_bmr tot_bmr_per tot_bmr_hi /* General_Purpose war_f1 */
{;
use temp1;
/* NON-PARAMETRIC DENSITY */;
kdensity `vio' if south==0, epanechnikov
	title("Non-parametric kernel density")
	saving(k_`vio'N, replace);
kdensity `vio' if south==1, epanechnikov
	title("Non-parametric kernel density")
	saving(k_`vio'S, replace);
kdensity `vio', epanechnikov
	title("Non-parametric kernel density")
	saving(k_`vio', replace);
*gen l_`vio' = log(`vio');
*kdensity l_`vio', epanechnikov
	title("Non-parametric kernel density")
	saving(k_l_`vio', replace);
*drop l_`vio';
clear;
...
foreach var in
	...
	poverty_p0 lit_rate elec_rate /* radio_rate urban_pct percent_cultivated */
	/* VLSS DATA */
	/* rlpcex1_9398 */ rlpcex1_93 rlpcex1 CONSGROWTHPC
	avg_ill4wks /* literate */ educyr98_head /* educyr98father educyr98mother */
	/* factory */ farm notbornhere /* yrshere */
{;

use temp1;
/* GRAPHICAL RELATIONSHIPS */;
/* OVERALL */;
*twoway (scatter `var' `vio', ms(Oh)) (lowess `var' `vio'), saving(`var'_`vio'_np, replace);
twoway (scatter `var' `vio', ms(Oh)) (lfit `var' `vio' [aw=pop_tot]), saving(`var'_`vio'_lin, replace);
*twoway (scatter `var' `vio', ms(Oh)) (lfit `var' `vio' [aw=pop_tot]) if sample_trim==1, saving(`var'_`vio'_linT, replace);
*twoway (scatter `var' `vio', ms(Oh)) (lfit `var' `vio' [aw=pop_tot]) if sample_restrict==1, saving(`var'_`vio'_linR, replace);
twoway (scatter `var' `vio', ms(Oh)) (lfit `var' `vio' [aw=pop_tot]) if south==0, saving(`var'_`vio'_linN, replace);
twoway (scatter `var' `vio', ms(Oh)) (lfit `var' `vio' [aw=pop_tot]) if south==1, saving(`var'_`vio'_linS, replace);
*twoway (scatter `var' `vio', ms(Oh)) (qfit `var' `vio' [aw=pop_tot]), saving(`var'_`vio'_sq, replace);
...
*twoway (scatter `var'_w `vio'_w, ms(Oh)) (lowess `var'_w `vio'_w), saving(`var'_`vio'_np_w, replace);
*twoway (scatter `var'_w `vio'_w, ms(Oh)) (lfit `var'_w `vio'_w [aw=pop_tot]), saving(`var'_`vio'_lin_w, replace);
*twoway (scatter `var'_w `vio'_w, ms(Oh)) (lfit `var'_w `vio'_w [aw=pop_tot]) if sample_trim==1, saving(`var'_`vio'_linT_w, replace);
*twoway (scatter `var'_w `vio'_w, ms(Oh)) (lfit `var'_w `vio'_w [aw=pop_tot]) if sample_restrict==1, saving(`var'_`vio'_linR_w, replace);
*twoway (scatter `var'_w `vio'_w, ms(Oh)) (lfit `var'_w `vio'_w [aw=pop_tot]) if south==0, saving(`var'_`vio'_linN_w, replace);
*twoway (scatter `var'_w `vio'_w, ms(Oh)) (lfit `var'_w `vio'_w [aw=pop_tot]) if south==1, saving(`var'_`vio'_linS_w, replace);
*twoway (scatter `var'_w `vio'_w, ms(Oh)) (qfit `var'_w `vio'_w [aw=pop_tot]), saving(`var'_`vio'_sq_w, replace);
drop *_w;

/* CONDITIONAL ON POPULATION DENSITY IN 1960 */;
quietly regress `vio' popdensity6061;
predict `vio'_pd, residual;
label var `vio'_pd "Conditional on 1960-1 Pop. Density, `vio'";

quietly regress `var' popdensity6061;
predict `var'_pd, residual;
label var `var'_pd "Conditional on 1960-1 Pop. Density, `var'";

*twoway (scatter `var'_pd `vio'_pd, ms(Oh)) (lowess `var'_pd `vio'_pd), saving(`var'_`vio'_np_pd, replace);
twoway (scatter `var'_pd `vio'_pd, ms(Oh)) (lfit `var'_pd `vio'_pd [aw=pop_tot]), saving(`var'_`vio'_lin_pd, replace);
*twoway (scatter `var'_pd `vio'_pd, ms(Oh)) (lfit `var'_pd `vio'_pd [aw=pop_tot]) if sample_trim==1,saving(`var'_`vio'_linT_pd, replace);
*twoway (scatter `var'_pd `vio'_pd, ms(Oh)) (lfit `var'_pd `vio'_pd [aw=pop_tot]) if sample_restrict==1,saving(`var'_`vio'_linR_pd, replace);
twoway (scatter `var'_pd `vio'_pd, ms(Oh)) (lfit `var'_pd `vio'_pd [aw=pop_tot]) if south==0, saving(`var'_`vio'_linN_pd, replace);
twoway (scatter `var'_pd `vio'_pd, ms(Oh)) (lfit `var'_pd `vio'_pd [aw=pop_tot]) if south==1, saving(`var'_`vio'_linS_pd, replace);
*twoway (scatter `var'_pd `vio'_pd, ms(Oh)) (qfit `var'_pd `vio'_pd [aw=pop_tot]), saving(`var'_`vio'_sq_pd, replace);
drop *_pd;
...
*twoway (scatter `var' `vio', ms(Oh)) (lowess `var' `vio'), saving(`var'_`vio'_np_b, replace);
twoway (scatter `var' `vio', ms(Oh)) (lfit `var' `vio' [aw=pop_prov]), saving(`var'_`vio'_lin_b, replace);
*twoway (scatter `var' `vio', ms(Oh)) (lfit `var' `vio' [aw=pop_prov]) if sample_trim==1, saving(`var'_`vio'_linT_b, replace);
*twoway (scatter `var' `vio', ms(Oh)) (lfit `var' `vio' [aw=pop_prov]) if sample_restrict==1, saving(`var'_`vio'_linR_b, replace);
twoway (scatter `var' `vio', ms(Oh)) (lfit `var' `vio' [aw=pop_prov]) if south==0, saving(`var'_`vio'_linN_b, replace);
twoway (scatter `var' `vio', ms(Oh)) (lfit `var' `vio' [aw=pop_prov]) if south==1, saving(`var'_`vio'_linS_b, replace);
*twoway (scatter `var' `vio', ms(Oh)) (qfit `var' `vio' [aw=pop_prov]), saving(`var'_`vio'_sq_b, replace);
clear;
...
/* REGRESSION ANALYSIS */;
/* OVERALL */;
regress `var' `vio' south [aw=pop_tot], robust cluster(province);
...
/* SPILLOVERS FROM OTHER DISTRICTS IN THE SAME PROVINCE */;
bys south: regress `var' `vio' `vio'_prov
	[aw=pop_tot], robust cluster(province);

/* ROBUST TO DROPPING 10 HIGHEST OUTLIERS, 11 LOWEST OUTLIERS */;
*areg `var' `vio'
	[aw=pop_tot]
	if sample_trim==1, a(province) robust cluster(province);
*bys south: areg `var' `vio'
	[aw=pop_tot]
	if sample_trim==1, a(province) robust cluster(province);

/* RESTRICT TO AREAS WITH BELOW/ABOVE AVERAGE BASELINE POPULATION DENSITY */;
/* MEDIAN POPULATION DENSITY 145 (UNITS?) */;
regress `var' `vio' south
	[aw=pop_tot]
	if popdensity6061<145, robust cluster(province);
bys south: regress `var' `vio'
	[aw=pop_tot]
	if popdensity6061<145, robust cluster(province);
/* INTERACTION EFFECT WITH INITIAL POPULATION DENSITY */;
gen `vio'_popdensity6061 = `vio'*popdensity6061;
regress `var' `vio'
	popdensity6061 `vio'_popdensity6061
	south
	[aw=pop_tot], robust cluster(province);
bys south: regress `var' `vio'
	popdensity6061 `vio'_popdensity6061
...
/* ADDITIONAL IMPACT OF LAND MINES */;
*areg `var' `vio' $ord3
	[aw=pop_tot], a(province) robust cluster(province);

/* RELATIONSHIP IN LOGS */;
gen l_`var' = log(`var');
gen l_`vio' = log(`vio');
...
regress `var' `vio' south [aw=pop_prov], robust;
