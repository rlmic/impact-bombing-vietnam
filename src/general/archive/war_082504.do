...
log using war_082504, t replace;
...
global x_oth = "ha_arable nav_riv_length dist_p_town_mean";
...
/**********HERE BEGIN NEW ADDITIONS: PRE-WAR DATA AND VLSS DATA**********/
...
/**********HERE ENDS NEW ADDITIONS, THOUGH COLLAPSING BELOW IS MODIFIED**********/



/* SUMMARY STATISTICS */;
summ 	poverty_p0 /* poverty_p1 poverty_p2 ineq_gini */ 	lit_rate /* elec_rate radio_rate
 tv_rate */
	/* urban_percent percent_cultivated */
	/* num_markets length_tot
 */
	$ord0 $ord1 $ord2 $ord3 /* $ord4 $ord5 $ord6 $ord7 $ord8 */
	tot_bmr tot_bomb war_f1 war_f2
	$x_weather $x_oth $x_soil1 $x_soil2 $x_elev $x_slope 
	popdensity* paddyyieldperhectare* births*
	/*WHATEVER ELSE YOU WANT*/
	pop_tot pop_prov [aw=pop_tot];

collapse
	poverty_p0 /* poverty_p1 poverty_p2 ineq_gini */ 
	lit_rate /* elec_rate radio_rate tv_rate */
	/* urban_percent percent_cultivated */
	/* num_markets length_tot */
	$ord0 $ord1 $ord2 $ord3 $ord4 $ord5 $ord6 $ord7 $ord8
	...
	$x_weather $x_oth $x_soil1 $x_soil2 $x_elev $x_slope 
	popdensity* paddyyieldperhectare* births*
	/*WHATEVER ELSE YOU WANT TO CHOOSE, THOUGH COLLAPSING COMMUNE-LEVEL VLSS DATA TO PROVINCE LEVEL MIGHT 
		BE OVERLY OPTIMISTIC ABOUT REPRESENTATIVE-NESS OF COMMUNE*/
	pop_prov [aw=pop_tot], by(province);
save temp2, replace;



**//PREVIOUS ANALYSES INCLUDED AS-IS AND DISABLED;
/*************clear;


foreach vio in
	tot_bmr /* tot_bomb */ war_f1 General_Purpose
{;
...
foreach var in
	poverty_p0 /* poverty_p1 poverty_p2 ineq_gini */ 	/* FUTURE VERSIONS - MORE POVERTY / INCOME MEASURES - 
	RESULTS HERE FOR LIT_RATE ARE VERY SIMILAR TO POVERTY */
	/* lit_rate elec_rate radio_rate
 tv_rate */
	/* urban_percent percent_cultivated */
	/* num_markets length_tot
 */
...
/* THESE CONTROLS WILL BE MORE USEFUL WHEN WE USE OTHER OUTCOME MEASURES - 
THE MINOT POVERTY MAP MEASURES USE THESE IN THE CONSTRUCTION OF THE POVERTY
MEASURE SO IT IS NO WONDER THEY HAVE SO MUCH PREDICTIVE POWER */;
areg `var' `vio'
	$x_weather $x_oth $x_soil1 $x_soil2 $x_elev $x_slope [aw=pop_tot],
